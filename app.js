document.addEventListener('DOMContentLoaded',function(){
  ...
  if(heroBg){
    ...
    var updateHero=function(){
      ...
      var candidates = isMobile ? ['herosection.png?v=1','herosection.jpg?v=1','hero_03_16x9.jpg?v=1'] : ['hero_03_16x9.jpg?v=1'];
      var imgs=[]; var loaded=[];
  ...
  try{
    if(window.firebaseUtil && window.firebaseUtil.enabled){
      var list=document.querySelector('.hours-list');
      if(list){
        window.firebaseUtil.loadHours().then(function(cfg){
          if(!cfg) return;
          list.querySelectorAll('li').forEach(function(li){
            var spans=li.querySelectorAll('span');
            if(spans.length<2) return;
  ...
  if(adminForm){
    ...
    if(saveCloudBtn){ saveCloudBtn.addEventListener('click', function(){ if(!(window.firebaseUtil && window.firebaseUtil.enabled)) return alert('Cloud disabled'); var u=window.firebaseUtil.user(); if(!u) return alert('Harap login dulu'); if(window.firebaseUtil.isOwner && !window.firebaseUtil.isOwner()) return alert('Akun tidak memiliki akses'); var cfg=readFormCfg(); if(!validateOpen(cfg)) return; window.firebaseUtil.saveHours(cfg).then(function(){ alert('jam berhasil disimpan') }).catch(function(){ alert('Gagal simpan') }); }); }
  ...
  if(menuBtn && mobileMenu){
    menuBtn.addEventListener('click',function(e){ e.stopPropagation(); mobileMenu.classList.toggle('open'); if(menuOverlay){ menuOverlay.classList.toggle('show', mobileMenu.classList.contains('open')); }});
    document.addEventListener('click',function(){ mobileMenu.classList.remove('open'); if(menuOverlay){ menuOverlay.classList.remove('show'); } });
